...
pid=$( curl $curlOpts --silent --user "${publicApiKey}:${privateApiKey}" --digest \
 ...
 --request GET "${opsMgrExtUrl}/api/public/v1.0/groups/byName/${project}?pretty=true" |jq .id )
...
curl $curlOpts --silent --user "${publicKey}:${privateKey}" --digest \
 --header "Accept: application/json" \
 --header "Content-Type: application/json" \
 --request GET "${opsMgrExtUrl}/api/public/v1.0/groups/${projectId}/logCollectionJobs?pretty=true" 
